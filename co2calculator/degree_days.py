"""Calculates degree days to enable comparison of heating emissions with other research groups located in different
climates or between different years.
"""
import numpy as np
import xarray as xr
...
def get_temp_series(
    ...
    """
    assert isinstance(
        start_time, (dtime, ddate)
    ), "start_time should be a datetime.date or datetime.datetime"
    assert isinstance(
        end_time, (dtime, ddate)
    ), "start_time should be a datetime.date or datetime.datetime"
    ...
    if isinstance(location, str):
        ...
        assert np.shape(location) == (
            2,
        ), "the given location coordinates are not the correct shape"
        coords = location
    else:
    ...
    ds = xr.open_dataset(
        fn
    )  # maybe in the future could optimize by not opening the entire file?
    # this takes from midnight of first day to midnight of first day of following month
    ds = (
        ds.sel(latitude=round(coords[1]))
        .sel(longitude=round(coords[0]))
...
def calc_degreedays(
    ...
    if dd_type not in ("heating", "cooling"):
        print(
            "WARNING calc_degreedays: degree days type not understood. Defaulting to type='heating'."
        )
        dd_type = "heating"
    ...
    if isinstance(date, (dtime, ddate)):
        ...
        )  # automatically sets to midnight of first day
    else:
    ...
    if dd["t2m"].values < 0.1:
        print(
            "WARNING calc_degreedays: degree days calculated to be <0.1, setting cutoff at 0.1."
        )
